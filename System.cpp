...
UWORD copper3[] __attribute__((section (".MEMF_CHIP"))) = {
...
APTR ThisSystem::GetInterruptHandler() {
	return *(volatile APTR*)(((UBYTE*)VBR)+0x6c);
}

USHORT* ThisSystem::screenScanDefault(USHORT* copListEnd) {
	const USHORT x=129;
...
USHORT* ThisSystem::copSetPlanes(UBYTE bplPtrStart,USHORT* copListEnd,const UBYTE **planes, USHORT i) {
...
USHORT* ThisSystem::copSetColor(USHORT* copListCurrent,USHORT index,USHORT color) {
	*copListCurrent++=offsetof(struct Custom, color[index]);
	*copListCurrent++=color;
	return copListCurrent;
}

void ThisSystem::FreeSystem() { 
	WaitVbl();
	WaitBlt();
	custom->intena=0x7fff;//disable all interrupts
	custom->intreq=0x7fff;//Clear any interrupts that were pending
...
void ThisSystem::WaitBlt() {
	UWORD tst=*(volatile UWORD*)&custom->dmaconr; //for compatiblity a1000
	(void)tst;
	while (*(volatile UWORD*)&custom->dmaconr&(1<<14)) {} //blitter busy wait
}

void ThisSystem::SetInterruptHandler(APTR interrupt) {
